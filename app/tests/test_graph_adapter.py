...
class TestGraphAdapter:
    ...
    async def test_add_relationship(self, adapter, sample_symbols):
        ...
        relationship = Relationship(
            source_id=sample_symbols[0].id, target_id=sample_symbols[1].id, type=RelationType.CALLS
        )
        await adapter.add_relationship(relationship)

        # Verify relationship exists
        relationships = await adapter.get_relationships(
            sample_symbols[0].id, RelationType.CALLS, direction="outgoing"
        )
    ...
    async def test_store_processes(self, adapter):
        ...
        await adapter.backend.register_repository(
            repo_id="test_repo", name="Test Repo", path="/test"
        )
    ...
    async def test_register_repository(self, adapter):
        ...
        await adapter.register_repository(repo_path="/test/repo", metadata={"branch": "main"})
    ...
    async def test_list_repositories(self, adapter):
        ...
        await adapter.register_repository(repo_path="/test/repo1", metadata={})
        await adapter.register_repository(repo_path="/test/repo2", metadata={})
    ...
    async def test_get_repository_stats(self, adapter, sample_symbols):
        ...
        # Register repo
        await adapter.backend.register_repository(repo_id=repo_id, name="Test Repo", path="/test")
