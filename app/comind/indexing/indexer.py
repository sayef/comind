...
class PythonSymbolExtractor:
    ...
    def _get_signature_from_source(self, node: Node, include_decorators: bool = True) -> str:
        ...
        for line_num in range(
            start_line, min(start_line + 10, len(self.lines))
        ):  # Limit to 10 lines
    ...
    def _extract_from_node(self, node: Node, parent_symbol: Symbol | None = None):
        ...
        if node.type == "module":
            ...
            if definition_node:
                if definition_node.type == "function_definition":
                    func_symbol = self._extract_function(
                        definition_node, parent_symbol, is_async=False, decorators=decorators
                    )
                    if func_symbol:
                        self.symbols.append(func_symbol)
                        if parent_symbol:
                    ...
                    class_symbol = self._extract_class(
                        definition_node, parent_symbol, decorators=decorators
                    )
                    if class_symbol:
                        self.symbols.append(class_symbol)
                        if parent_symbol:
    ...
    def _extract_class(
        self, node: Node, parent: Symbol | None = None, decorators: list[str] = None
    ) -> Symbol:
    ...
    def _extract_function(
        ...
        if params_node:
            for child in params_node.children:
                if child.type == "identifier":
                    ...
                    # *args or **kwargs
                    param_name_node = child.child_by_field_name("name") or (
        ...
        symbol_type = (
            SymbolType.METHOD if parent and parent.type == SymbolType.CLASS else SymbolType.FUNCTION
        )
    ...
    def _process_imports(self):
        ...
        for module_name, alias_name, line in self.imports:
            ...
            self.relationships.append(
                Relationship(
                    source_id=module_id, target_id=import_symbol.id, type=RelationType.IMPORTS
                )
            )
...
class PythonIndexer:
    ...
    def __init__(self, graph: KnowledgeGraph, query_engine=None):
        ...
        self.current_repo_id: str | None = None  # Track current repository being indexed
        self.repo_root: Path | None = None  # Root directory of the repository being indexed
        self.gitignore_handler: GitignoreHandler | None = None  # Gitignore pattern matcher
    ...
    async def index_file(
        ...
        if self.query_engine and self.current_repo_id:
            for symbol in symbols:
                ...
                await self.query_engine.add_symbol_to_index(self.current_repo_id, symbol, content)
    ...
    async def index_directory(
        ...
        if self.gitignore_handler:
            python_files = [f for f in python_files if not self.gitignore_handler.should_ignore(f)]
        ...
        if skipped_files:
            logger.info(
                ...
                reasons={
                    r: sum(1 for _, reason in skipped_files if reason == r)
                    for _, r in skipped_files
                },
            )
        ...
        logger.debug(
            "Bulk inserting symbols and relationships",
        ...
        if processes:
            ...
            for p in processes:
                ...
                if trace:
                    ...
                    for query in queries:
                        all_process_queries.append({"process_id": p.id, "query": query})
            ...
            if all_process_queries and self.query_engine:
                ...
                logger.debug(
                    f"Generating embeddings for {len(all_process_queries)} process queries..."
                )
