...
class IncrementalIndexer:
    ...
    async def process_changes(
        self, repo_id: str, repo_path: str, changed_files: dict[str, str], indexer_func
    ) -> tuple[int, int, int]:
    ...
    async def should_regenerate_llm_content(self, symbol: Symbol, cache_type: str) -> bool:
    ...
    async def get_symbols_needing_llm_generation(
        self, symbols: list[Symbol], cache_type: str
    ) -> list[Symbol]:
    ...
    async def save_llm_result(
        self, symbol: Symbol, cache_type: str, content: str, model: str, metadata: dict = None
    ) -> None:
    ...
    async def get_incremental_stats(self, repo_id: str) -> dict[str, any]:
        ...
        cache_info = {
            row[0]: {"entries": row[1], "total_accesses": row[2], "avg_accesses": row[3]}
...
async def smart_reindex(
    ...
    if force:
        ...
        changed_files = dict.fromkeys(all_files, "modified")
    else:
        # Detect changes
    ...
    if not changed_files:
        ...
        return {"changed": False, "files_processed": 0, "symbols_added": 0}
