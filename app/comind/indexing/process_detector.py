...
from comind.core.graph import Symbol, SymbolType, RelationType
...
class ProcessDetector:
    ...
    async def _trace_execution_from_entry(self, entry_point: Symbol) -> List[List[Symbol]]:
        ...
        while queue and len(traces) < self.config.max_processes:
            ...
            if callees:
                ...
                if len(current_trace) >= self.config.min_steps:
    ...
    async def _create_process_node(self, trace: List[Symbol]) -> Optional[ProcessNode]:
    ...
    async def get_process_by_id(self, process_id: str) -> Optional[ProcessNode]:
