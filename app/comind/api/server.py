...
from comind.wiki.wiki import WikiGenerator, generate_wiki as generate_wiki_docs, load_wiki_pages
...
def clone_or_pull_git_repo(repo_url: str, repo_name: str, branch: str = "main") -> tuple[str, bool]:
    ...
    if gitlab_token and "gitlab.com" in repo_url:
        ...
        if clean_url.startswith("https://gitlab.com"):
            clone_url = clean_url.replace(
                "https://gitlab.com",
                f"https://imsaif:{gitlab_token}@gitlab.com"
            )
    ...
    try:
        if repo_dir.exists() and (repo_dir / ".git").exists():
            ...
            logger.info("Cloning git repository", url=repo_url, branch=branch, repo_name=repo_name, path=str(repo_dir))
            ...
            try:
                ...
                logger.info("Git repository cloned successfully", repo_name=repo_name, path=str(repo_dir))
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to clone/update repository: {str(e)}"
        )
...
class IndexRequest(BaseModel):
    ...
    repo: str = Field(..., description="Repository path or Git URL (e.g., '/path/to/repo' or 'https://gitlab.com/user/repo.git')")
...
def create_app() -> FastAPI:
    ...
    async def analyze_repository(request: IndexRequest) -> dict[str, Any]:
        ...
        if is_git:
            ...
            logger.info(f"Repository {action}", url=request.repo, branch=request.branch, path=repo_path, repo_name=repo_name)
        else:
            # Use local path
            repo_path = request.repo
            logger.info("Using local repository", path=repo_path, repo_name=repo_name)
        
        logger.info("Starting complete repository analysis", repo_path=repo_path, repo_name=repo_name, is_git=is_git)
        ...
        try:
            ...
            logger.info(
                "Phase 3 complete",
                pages=wiki_result.pages_generated,
                mode=wiki_result.mode
            )
            ...
            await generate_style_guide_markdown(
                style_patterns,
                repo_name,
    ...
    async def search_code(
        ...
        logger.info("Searching code", repo_name=repo_name, query=query, case_sensitive=case_sensitive)
        ...
        try:
            ...
            if result.returncode not in (0, 1):
                raise HTTPException(
                    status_code=500,
                    detail=f"Grep search failed: {result.stderr}"
                )
    ...
    async def generate_style_guide(repo_name: str) -> dict[str, Any]:
        ...
        markdown = await generate_style_guide_markdown(
            patterns,
            repo_name,
            str(output_path)
        )
        ...
        return {
            ...
            "patterns_analyzed": {
                ...
                "dominant_naming": {
                    "functions": patterns.function_naming.most_common(1)[0][0] if patterns.function_naming else "snake_case",
                    "classes": patterns.class_naming.most_common(1)[0][0] if patterns.class_naming else "PascalCase"
                }
            },
    ...
    async def search(request: SearchRequest):
        ...
        """
        start_time = time.time()
        logger.info("Search request", query=request.query, repo_name=request.repo_name, format=request.format)
        
        # Enforce repo_name requirement
        if not request.repo_name:
            raise HTTPException(
                status_code=400,
                detail="repo_name is required - global search is not supported"
            )
    ...
    async def get_wiki(repo_id: str) -> dict[str, Any]:
        ...
        if not pages:
            raise HTTPException(status_code=404, detail="Wiki not found. Generate it first with POST /api/wiki/generate")
    ...
    async def get_graph_schema() -> dict[str, Any]:
        ...
        return {
            "nodes": {
                "types": ["file", "module", "class", "function", "method", "variable", "import", "community", "process"],
                "properties": ["id", "name", "type", "file_path", "line_start", "line_end", "signature", "docstring"]
            },
            "relationships": {
                "types": ["contains", "imports", "calls", "inherits", "implements", "member_of", "participates_in", "defines", "uses"],
    ...
    async def get_graph_stats() -> dict[str, Any]:
        ...
        try:
            stats_row = graph.backend.conn.execute(
                "SELECT COUNT(*) FROM symbols"
            ).fetchone()
            symbol_count = stats_row[0] if stats_row else 0
            
            rel_row = graph.backend.conn.execute(
    ...
    async def handle_mcp_message(message: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "id": message.get("id"),
            "result": {
                "message": "MCP over HTTP not fully implemented yet",
                "received": message
            }
        }
