...
from comind.core.graph import GraphBackend, Symbol
...
class QueryGenerator:
    ...
    async def generate_queries_for_symbol(
        self, symbol: Symbol, code_snippet: str = "", max_queries: int = 10
    ) -> list[str]:
        """
        ...
        try:
            ...
            logger.debug(
                "Generated queries for symbol", symbol=symbol.name, query_count=len(queries)
            )

            return queries

        except Exception as e:
            logger.error("Failed to generate queries for symbol", symbol=symbol.name, error=str(e))
    ...
    async def generate_queries_batch(
        self, symbols: list[Symbol], code_snippets: dict[str, str] = None, batch_size: int = 20
    ) -> dict[str, list[str]]:
        """
