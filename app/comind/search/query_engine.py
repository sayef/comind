...
class TextSearchEngine:
    ...
    async def search(self, query: str, limit: int = 10) -> List[Tuple[str, float]]:
        ...
        return [(self.symbol_ids[idx], float(scores[0][i])) 
...
class SemanticSearchEngine:
    ...
    async def build_index(self):
        ...
        for i, (symbol_id, embedding) in enumerate(zip(symbol_ids, embeddings_gen)):
            ...
            if processed % 500 == 0 or processed == total:
                logger.info(f"Semantic embeddings progress: {processed}/{total} ({100*processed//total}%)")
    ...
    async def load(self, index_dir: str) -> bool:
        ...
        if isinstance(data, dict) and "embeddings" in data:
            ...
            logger.info(f"Loaded {len(self.embeddings)} embeddings ({stored_dim}-dim, {stored_version})")
        else:
...
class WikiStore:
    ...
    async def add_page(self, page_id: str, title: str, content: str, metadata: Dict[str, Any] = None, mentioned_symbols: List[str] = None):
    ...
    async def get_relevant_section(self, page_id: str, query: str) -> Optional[str]:
    ...
    async def save(self, index_dir: str):
        ...
        data = {
            "pages": self.pages,
            "page_symbols": self.page_symbols
        }
...
class WikiEnhancedQueryEngine:
    ...
    async def search(
        ...
        if not repo_id:
            return {
                "error": "repo_id is required - global search is not supported",
                "results": []
            }
        ...
        if not text_engine:
            return {
                "error": f"Repository {repo_id} not indexed",
                "results": []
            }
    ...
    async def analyze_impact(
        ...
        total_affected = (
            len(impact["upstream"]["depth_1"]) + 
            len(impact["downstream"]["depth_1"])
        )
    ...
    async def _merge_results(
        ...
        text_rankings = {symbol_id: rank for rank, (symbol_id, _) in enumerate(text_results) if isinstance(symbol_id, str)}
        if text_rankings:
            ranked_lists["text"] = text_rankings
        
        # Semantic search rankings
        semantic_rankings = {symbol_id: rank for rank, (symbol_id, _) in enumerate(semantic_results) if isinstance(symbol_id, str)}
        ...
        wiki_rankings = {symbol_id: rank for rank, (symbol_id, _) in enumerate(wiki_results) if isinstance(symbol_id, str)}
        if wiki_rankings:
    ...
    async def _find_symbol(self, name: str, file_path: Optional[str] = None) -> Optional[Symbol]:
    ...
    async def _get_graph_context(self, symbol: Symbol) -> Dict[str, Any]:
        ...
        return {
            ...
            "dependencies": [{"name": d.name, "file": d.file_path} for d in unique_dependencies.values()][:MAX_GRAPH_ITEMS]
        }
    
    async def _get_wiki_context(self, symbol: Symbol, query: str, repo_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _trace_upstream(self, symbol_id: str, impact: Dict, max_depth: int, min_confidence: float, current_depth: int = 1):
        ...
        for caller in callers:
            ...
            if caller.id not in [s["id"] for s in impact["upstream"][depth_key]]:
                ...
                # Recurse
                await self._trace_upstream(caller.id, impact, max_depth, min_confidence, current_depth + 1)
    
    async def _trace_downstream(self, symbol_id: str, impact: Dict, max_depth: int, min_confidence: float, current_depth: int = 1):
        ...
        for callee in callees:
            ...
            if callee.id not in [s["id"] for s in impact["downstream"][depth_key]]:
                ...
                # Recurse
                await self._trace_downstream(callee.id, impact, max_depth, min_confidence, current_depth + 1)
    
    async def _trace_execution_flow(self, symbol_id: str, flow: Dict, max_depth: int, current_depth: int):
