...
class DuckDBSemanticSearchEngine:
    ...
    def __init__(self, backend, embedding_model: Optional[str] = None):
        """Initialize with DuckDB backend
    ...
    async def generate_embeddings_for_repo(self, repo_id: str, batch_size: int = 64, include_enriched: bool = False) -> int:
        ...
        for batch_num, batch_start in enumerate(range(0, total, batch_size), 1):
            ...
            if batch_num % log_interval == 0 or batch_num == total_batches:
                logger.debug(f"Generating{enriched_str} embeddings for {total} symbols: {batch_num}/{total_batches} batches, {stored} embedded")
...
def create_search_engines(backend, embedding_model: Optional[str] = None):
