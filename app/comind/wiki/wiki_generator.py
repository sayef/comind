...
from comind.llm.llm_client import LLMConfig, call_llm, estimate_tokens
...
class WikiRunResult:
    ...
    module_tree: Optional[List[ModuleTreeNode]] = None
...
class WikiGenerator:
    ...
    def _progress(self, phase: str, percent: int, detail: Optional[str] = None):
        """Report progress"""
        if percent > 0:
            self.last_percent = percent
        self.on_progress(phase, percent, detail)
    
    def _stream_opts(self, label: str, fixed_percent: Optional[int] = None, percent_range: int = 10):
    ...
    async def _full_generation(self, current_commit: str) -> WikiRunResult:
        ...
        enriched_files = [
            export_map.get(fp, {"file_path": fp, "symbols": []})
        ...
        def report_progress(module_name: Optional[str] = None):
            nonlocal modules_processed
            modules_processed += 1
            percent = 30 + int((modules_processed / total_modules) * 55)
            detail = f"{modules_processed}/{total_modules} — {module_name}" if module_name else f"{modules_processed}/{total_modules} modules"
        ...
        await self._save_wiki_meta(WikiMeta(
    ...
    async def _build_module_tree(self, files: List[Dict[str, Any]]) -> List[ModuleTreeNode]:
        ...
        prompt = fill_template(GROUPING_USER_PROMPT, {
            "FILE_LIST": file_list,
    ...
    async def _generate_leaf_page(self, node: ModuleTreeNode):
        ...
        # Build prompt
        prompt = fill_template(MODULE_USER_PROMPT, {
    ...
    async def _generate_parent_page(self, node: ModuleTreeNode):
        ...
        # Build prompt
        prompt = fill_template(PARENT_USER_PROMPT, {
            "MODULE_NAME": node.name,
            "CHILDREN_DOCS": "\n\n".join(children_docs),
            "CROSS_MODULE_CALLS": format_call_edges(inter_module["outgoing"] + inter_module["incoming"]),
    ...
    async def _generate_overview(self, module_tree: List[ModuleTreeNode]):
        ...
        prompt = fill_template(OVERVIEW_USER_PROMPT, {
    ...
    def _flatten_module_tree(self, tree: List[ModuleTreeNode]) -> tuple[List[ModuleTreeNode], List[ModuleTreeNode]]:
    ...
    async def _load_wiki_meta(self) -> Optional[WikiMeta]:
        """Load wiki metadata"""
