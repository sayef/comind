...
from comind.core.graph import Symbol, SymbolType, Relationship, RelationType
...
class DuckDBBackend:
    ...
    async def get_symbol(self, symbol_id: str) -> Optional[Symbol]:
        """Get a symbol by ID"""
    ...
    def _cascade_delete_symbol_deps(self, symbol_ids: List[str]) -> None:
        ...
        self.conn.execute(
            f"DELETE FROM file_symbols WHERE symbol_id IN ({ph})", ids
        )
        self.conn.execute(
            f"DELETE FROM symbol_embeddings WHERE symbol_id IN ({ph})", ids
        )
        self.conn.execute(
            f"DELETE FROM llm_cache WHERE symbol_id IN ({ph})", ids
        )
        self.conn.execute(
    ...
    async def add_symbols_batch(self, symbols: List[Symbol]) -> None:
        ...
        data = [
            (
                s.id, s.name, s.type.value, s.file_path, s.line_start, s.line_end,
                s.repo_id, s.signature, s.docstring, s.description,
                json.dumps(s.properties) if s.properties else None,
                json.dumps(s.associated_queries) if s.associated_queries else None,
                s.wiki_page_id, s.wiki_section
            )
            for s in symbols
        ]
    ...
    async def add_relationships_batch(self, relationships: List[Relationship]) -> None:
        ...
        data = [
            (
                rel.source_id, rel.target_id, rel.type.value,
                rel.confidence,
    ...
    async def add_relationship(self, relationship: Relationship) -> None:
        ...
        if not source_exists:
            logger.warning(f"Skipping relationship: source symbol not found", source_id=relationship.source_id)
            return
        if not target_exists:
            logger.debug(f"Skipping relationship: target symbol not found (likely external/builtin)", 
                        target_id=relationship.target_id, type=relationship.type.value)
            return
    ...
    async def get_callers(self, symbol_id: str, repo_id: Optional[str] = None) -> List[Symbol]:
    ...
    async def get_callees(self, symbol_id: str, repo_id: Optional[str] = None) -> List[Symbol]:
    ...
    async def update_file_metadata(
        ...
        self.conn.execute(
            "DELETE FROM file_symbols WHERE file_path = ?", (file_path,)
        )
        self.conn.execute("""
    ...
    async def get_llm_cache(
        ...
        """, (cache_key, content_hash)).fetchone()
    ...
    async def semantic_search(
        ...
        try:
            result = self.conn.execute("""
                ...
                LIMIT ?
            """, (query_embedding.tolist(), min_similarity, limit)).fetchall()
            
            return [
                (self._row_to_symbol(row[:-1]), row[-1])
                for row in result
            ]
        except Exception as e:
    ...
    async def register_repository(
        ...
        if existing:
            ...
            self.conn.execute(
                "DELETE FROM processes WHERE repo_id = ?", (repo_id,)
            )
    ...
    async def get_repository_stats(self, repo_id: str) -> Optional[Dict[str, Any]]:
